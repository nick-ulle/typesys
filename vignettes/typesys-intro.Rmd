...
`RInteger` and `RLogical`. Unlike R's `typeof()` function, we distinguish
between scalars and vectors. Constant types always describe scalar values. For
example, `RInteger` is a scalar integer. Generally, the names of the constant
types match the names returned by `typeof()` up to capitalization. There are
two exceptions:
...
If we want to create a new `RInteger` type, we can write
```{r}
RInteger
```
The output of this function is an S4 class that represents an `RInteger`.
